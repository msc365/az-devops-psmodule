...
Describe 'New-AdoCheckBusinessHours' {
    BeforeAll {
        ...
        $mockCreatedConfiguration = @{
            ...
            settings  = @{
                ...
                definitionRef = @{ id = 'b2d59b0e-b0b0-4f4f-9f4f-4f4f4f4f4f4f'; name = 'BusinessHours' }
            }
    ...
    Context 'Core Functionality Tests' {
        ...
        It 'Should create business hours check with default parameters' {
            # Act
            $result = New-AdoCheckBusinessHours -CollectionUri 'https://dev.azure.com/my-org' -ProjectName 'TestProject' -ResourceType 'environment' -ResourceName 'TestEnvironment' -Confirm:$false
        ...
        It 'Should create business hours check with custom parameters' {
            # Act
            $result = New-AdoCheckBusinessHours -CollectionUri 'https://dev.azure.com/my-org' -ProjectName 'TestProject' -DisplayName 'Custom Business Hours' -ResourceType 'environment' -ResourceName 'TestEnvironment' -BusinessDays @('Monday', 'Wednesday', 'Friday') -TimeZone 'Pacific Standard Time' -StartTime '08:00' -EndTime '17:00' -Timeout 2880 -Confirm:$false
        ...
        It 'Should call Get-AdoEnvironment to resolve resource ID' {
            # Act
            New-AdoCheckBusinessHours -CollectionUri 'https://dev.azure.com/my-org' -ProjectName 'TestProject' -ResourceType 'environment' -ResourceName 'TestEnvironment' -Confirm:$false

            # Assert
            Should -Invoke Get-AdoEnvironment -ModuleName Azure.DevOps.PSModule -Times 1
        }

        It 'Should accept multiple business days' {
            # Act
            $result = New-AdoCheckBusinessHours -CollectionUri 'https://dev.azure.com/my-org' -ProjectName 'TestProject' -ResourceType 'environment' -ResourceName 'TestEnvironment' -BusinessDays @('Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday') -Confirm:$false
    ...
    Context 'Input Validation Tests' {
        ...
        It 'Should validate CollectionUri format' {
            # Act & Assert
            { New-AdoCheckBusinessHours -CollectionUri 'invalid-uri' -ProjectName 'TestProject' -ResourceType 'environment' -ResourceName 'TestEnv' -Confirm:$false } | Should -Throw
        }
    }
    ...
    Context 'Error Handling Tests' {
        ...
        It 'Should throw error for unsupported resource types' {
            ...
            # Act & Assert
            { New-AdoCheckBusinessHours -CollectionUri 'https://dev.azure.com/my-org' -ProjectName 'TestProject' -ResourceType 'endpoint' -ResourceName 'TestEndpoint' -Confirm:$false } | Should -Throw '*not supported yet*'
        }
